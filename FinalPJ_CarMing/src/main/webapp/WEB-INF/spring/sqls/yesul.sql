...
SELECT COUNT(BRDNO) COUNT FROM
	...
	update member set memfile = 'resources/img/profile.png' where pno=4
		...
		JOIN PAY PY
		ON C.CARTNO = PY.CARTNO
		WHERE PCATEGORYNAME LIKE  '%%' OR MEMNICK LIKE '%%'
		ORDER BY PY.PAYNO DESC) REN)
		WHERE RNUM BETWEEN 1 + (1-1) * 6 AND 1  * 6
		...
		update pay set payno = 1 where cartno = 9
		...
		select * from cart where cartno=69
		
		update cart set enddate= date'2021-02-13' where cartno=69
		DATE '2021-01-23'
		
		update cart set memno = 3
		
		select * from RENTSTATE
		
		select * from product 
		
		update product set pprice= 50000 where pno = 4
